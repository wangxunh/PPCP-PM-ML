# -*- coding: utf-8 -*-
"""
Created on Mon May 27 20:40:41 2024
...
# (optional) names if you need them later
feature_names = numeric_cols + list(encoded_feature_names)

# 5) NOTE: Do NOT standardize the full matrix here (avoids leakage).
#    Only numeric columns will be z-scored inside each CV fold.

# 11. K-Fold cross-validation setup
kf = KFold(n_splits=10, shuffle=True)

# numeric indices = the first len(numeric_cols) columns in X
...
for run in range(n_runs):
    ...
    plt.title(f'$R^2$ = {r2:.3f}, MAE = {mae:.3f}, RMSE = {rmse:.3f}, SVM', fontsize=11)
