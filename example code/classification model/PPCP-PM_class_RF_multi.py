# -*- coding: utf-8 -*-
"""
Created on Tue May 28 22:04:24 2024
...
for run in range(n_runs):
    ...
    for train_index, test_index in kf.split(features):
        ...
        for p in rf_param_list:
            model_val = RandomForestClassifier(**p)
            model_val.fit(X_tr, y_tr)
            try:
                val_probs = model_val.predict_proba(X_val)
                score = roc_auc_score(to_categorical(y_val, num_classes=3), val_probs, multi_class='ovr')
            except ValueError:
        ...
        sensitivity, specificity = calculate_sensitivity_specificity(test_targets, predicted_labels)
        balanced_accuracy = (sensitivity + specificity) / 2
        auroc = roc_auc_score(to_categorical(test_targets, num_classes=3), probabilities, multi_class='ovr')
        f1 = f1_score(test_targets, predicted_labels, average='weighted')
