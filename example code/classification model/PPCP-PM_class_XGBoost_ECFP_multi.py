# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 20:30:36 2024
...
# Indices for numeric block inside the combined feature matrix
n_ecfp = ecfp_features.shape[1]
n_num  = len(numeric_cols)
num_start = n_ecfp
num_end   = n_ecfp + n_num  # exclusive
...
for run in range(n_runs):
    ...
    # NOTE: split on the unscaled feature matrix to avoid leakage
    for train_index, test_index in kf.split(features):
        train_features, test_features = features[train_index].copy(), features[test_index].copy()
        ...
        # ---- Scale ONLY numeric features, fit scaler on training fold ONLY ----
        scaler = StandardScaler()
        train_features[:, num_start:num_end] = scaler.fit_transform(train_features[:, num_start:num_end])
        test_features[:,  num_start:num_end] = scaler.transform(test_features[:,  num_start:num_end])
