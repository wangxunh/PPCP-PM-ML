# -*- coding: utf-8 -*-
"""
Created on Fri May 24 16:54:06 2024
...
# Drop the original categorical columns from the data and merge with the one-hot encoded data
data = data.drop(columns=categorical_cols)
data = pd.concat([data, df_encoded], axis=1)

# Define the numeric base features (ensure these column names match your dataset)
numeric_cols = [
    'logP', 'MW', 'Original concentration (mg/L)', 'pH', 'Dark time', 'Light time'
]

# Extract ECFP features, numeric features, and one-hot encoded features
ecfp_features = np.array(data['ECFP'].tolist())
numeric_features = data[numeric_cols].values
one_hot_features = data[encoded_feature_names].values
...
# Initialize storage for metrics across multiple runs
run_results = {
    'Accuracy': [],
    'Sensitivity': [],
    'Specificity': [],
